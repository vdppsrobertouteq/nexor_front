// File: frontend/src/pages/Superadministrador/SuperAdminDashboard/DashboardMetricsContent.jsx
import React, { useState, useEffect } from 'react';
import {
  Card, Row, Col, Statistic,
  Table, List, Spin, Tag, Badge, Typography, Space,
...
import superadminMetricsService from '../../../services/superadminMetricsService';
...
const DashboardMetricsContent = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // 1. Resumen general
      const summaryRes = await superadminMetricsService.getGeneralSummary();
      setGeneralSummary({
      ...
      const [
        ...
        superadminMetricsService.getProjectsStatusDistribution(),
        superadminMetricsService.getProjectsRiskDistribution(),
        superadminMetricsService.getProjectsByPortfolio(),
        superadminMetricsService.getTasksStatusDistribution(),
        superadminMetricsService.getTasksPriorityDistribution(),
        superadminMetricsService.getDocumentsSignedStats()
      ]);
      ...
      const [
        ...
        superadminMetricsService.getProjectsWithPendingTasks(),
        superadminMetricsService.getPendingDocuments(),
        superadminMetricsService.getScheduledMeetings('today')
      ]);
  ...
  const loadKpis = async (days = 30) => {
    setLoadingKpis(true);
    try {
      const res = await superadminMetricsService.getKPIsByPeriod(days);
  ...
  return (
    <div className="dashboard-container">
      ...
      <Row gutter={[16, 16]} className="charts-section">
        ...
        <Col xs={24} lg={12}>
          <Card title="Documentos: Estado de Firma" className="chart-card">
            <Spin spinning={loadingCharts}>
